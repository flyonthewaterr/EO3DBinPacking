...
class SuperitemPool:
    ...
    def gen_superitems(
        ...
        superitemsing = cls._gen_single_items_superitems(items)
        if only_single:
        ...
        if horizontal:
            ...
            superitems += cls._gen_superitems_horizontal(superitemsing, htype=horizontal_type)
        ...
        superitems += cls._gen_superitems_vertical(superitemsing, max_vstacked)
# wwwww here
        superitems += superitemsing
    ...
    def _gen_single_items_superitems(cls, items):
        ...
        # Group superitems by volume
        groups = {}
        for superitem in superitems:
            volume = superitem.volume
            if volume not in groups:
                groups[volume] = []
            groups[volume].append(superitem)

        # Divide adjacent types into groups of two
        keys = sorted(groups.keys())
        divided_groups = []
        for i in range(0, len(keys), 2):
            if i + 1 < len(keys):
                divided_groups.append((keys[i], keys[i + 1]))
            else:
                divided_groups.append((keys[i],))

        # Combine adjacent groups into groups of two and shuffle
        random_superitems = []
        for group in divided_groups:
            if len(group) == 1:
                random_superitems += groups[group[0]]
            else:
                combined_superitems = [item for volume in group for item in groups[volume]]
                random.shuffle(combined_superitems)
                random_superitems += combined_superitems

        # Return combined groups as new list of Superitem objects
        superitems = random_superitems
    ...
    def _gen_superitems_vertical(cls, superitems, max_vstacked, tol=0.7):
        ...
        def _gen_superitems_vertical_subgroup(superitems):
            ...
            # Group slices based on objects and shuffle groups of adjacent types
            grouped_slices = {}
            for s in slices:
                key = tuple(s)
                if key not in grouped_slices:
                    grouped_slices[key] = [s]
                else:
                    grouped_slices[key].append(s)

            # Divide slice tuples into groups of adjacent types
            slice_groups = []
            keys = list(grouped_slices.keys())
            for i in range(0, len(keys), 2):
                if i + 1 < len(keys):
                    slice_groups.append((keys[i], keys[i + 1]))
                else:
                    slice_groups.append((keys[i],))

            # Combine and shuffle slice tuples of adjacent types
            rand_slices = []
            for i, group in enumerate(slice_groups):
                if len(group) == 1:
                    rand_slices += grouped_slices[group[0]]
                else:
                    combined_slices = grouped_slices[group[0]] + grouped_slices[group[1]]
                    random.shuffle(combined_slices)
                    rand_slices += combined_slices

            # Generate vertical superitems from each slice in rand_slices
            subgroup_vertical = []
            for slice_tuple in rand_slices:
                subgroup_vertical += [VerticalSuperitem(slice_tuple)]
