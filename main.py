...
def maxrects_warm_start(superitems_pool, height_tol=0, density_tol=0.5, add_single=True):
    ...
    # Modify the above code block as follows:
    height_groups = [superitems_pool] if len(superitems_pool) > 0 else []
    if len(height_groups) == 0:
        logger.debug(f"MR found no height groups, falling back to standard procedure")
        height_groups = [superitems_pool]
...
def main(
    ...
    # Create the final(initial here) superitems pool and a copy of the order
    ...
    for iter in range(max_iters):
        ...
        # and call the baseline procedure?
        # the all_singles_removed are not in horizontal superitems
