...
class BinPool:
    ...
    def get_volumes(self):
        """
        Return the list of bin volumes in the pool
        """
        return [b.volume for b in self.bins]
# wwwww--here
    ...
    def get_bin_densities(self):
        """
        Return the 3D densities for each bin
        """
...
# ---------wwwwhere
    def describe(self):
        """
        Return a DataFrame with stats about the current bin pool
        """
        ids = list(range(len(self.bins)))
        heights = self.get_heights()
        volumes = self.get_volumes()
        densities_3d = self.get_bin_densities()
        df = pd.DataFrame(
            zip(ids, heights,  volumes, densities_3d),
            columns=["bin", "height", "volume", "3d_density"],
        )
        total = (
            df.agg(
                {
...
class CompactBinPool:
    ...
    def __init__(self, bin_pool):
        ...
        self.compact_bins_num = len(self.compact_bins)
